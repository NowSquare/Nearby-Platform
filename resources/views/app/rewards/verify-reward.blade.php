          ...
          <div class="mt-4 mt-md-5">
            ...
            @if ($errors->any())
              <div class="alert alert-danger mb-3 mt-4 rounded-0">
                {{ trans('nearby-platform.redeem_reward_failed_text') }}
            ...
            <p class="lead mt-3">{{ trans('nearby-platform.redeem_reward_text') }}</p>

            <form class="form-horizontal" method="POST" action="{{ url('reward/verify/' . $reward_hash) }}" autocomplete="off">
              {{ csrf_field() }}
...
?>
              <div class="form-group row">
                ...
                <div class="col-12">
                  ...
                  <small class="form-text text-muted">{{ trans('nearby-platform.redeem_reward_code_help') }}</small>
              ...
              <button type="submit" class="mt-5 btn rounded-0 btn-{{ $reward->primaryColor }} btn-xlg btn-block text-truncate">{!! $reward->redeemBtnText !!}</button>
            </form>
          </div>
        </div>
      </div>
