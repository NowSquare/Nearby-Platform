      ...
      <div class="row">
        <div class="col-12">
          <div class="mt-4 mt-md-5">
            ...
            <p class="lead mt-3 text-center">{!! $coupon->redeemText !!}</p>

            <img src="{{ DNS2D::getBarcodePNGPath($redeem_url, 'QRCODE', 20, 20, [0,0,0]) }}" class="img-thumbnail img-fluid my-1 mdl-shadow--2dp p-3" alt="<?php echo str_replace('"', '&quot;', $coupon->title); ?>" style="min-width: 100%">
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  channel.bind('redeemed', function(data) {
    document.location.replace("{{ url('coupon/redeemed/' . $coupon_hash) }}");
  });
</script>
