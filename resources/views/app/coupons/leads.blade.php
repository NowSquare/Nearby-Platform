...
@section('page_title'){{ trans('nearby-platform.coupons') }} \ {{ trans('nearby-platform.leads') }} - {{ config()->get('system.premium_name') }} @stop
    ...
    <div class="content text-dark" style="background-image: url('{{ url('assets/images/backgrounds/triangles.jpg') }}');">
      <div class="content-overlay" style="background-color:rgba(245,249,250,0.9)">
        <div class="container">
          <div class="row">
            <div class="col-md-8">
              <h1 class="mb-0">{{ trans('nearby-platform.coupons') }} \ {{ trans('nearby-platform.leads') }} ({{ $leads->total() }})</h1>
            </div>
          </div>
        </div>
      </div>
              ...
              <div class="col-12 col-md-6 text-left text-md-right mt-2 mt-md-0">
                ...
                <span class="dropdown">
                  ...
                  <div class="dropdown-menu rounded-0 mdl-shadow--2dp dropdown-menu-right">
                    <a class="dropdown-item<?php echo (count($leads) > 0) ? '' : ' disabled'; ?>" href="<?php echo (count($leads) > 0) ? url('dashboard/coupons/leads/export?sortby=' . $sortby . '&order=' . $order . '&' . http_build_query($qsFilterCoupons)) : 'javascript:void(0);'; ?>">{{ trans('nearby-platform.export_leads') }}</a>
                  </div>
                </span>
          ...
          <div class="row">
            <div class="col-12">
              ...
              <ul class="nav nav-tabs">
                <li class="nav-item">
                  <a class="nav-link color-dark rounded-0" href="{{ url('dashboard/coupons') }}">{{ trans('nearby-platform.coupons') }}</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link active rounded-0" href="{{ url('dashboard/coupons/leads') }}">{{ trans('nearby-platform.leads') }}</a>
                </li>
              </ul>
                ...
                <div class="row">
                  <div class="col-12 col-md-12">
                    ...
                    <div class="input-group mb-3 mdl-shadow--2dp">
                      <select multiple class="form-control select2-multiple" size="1" name="filterCoupons[]">
<?php
foreach ($coupons as $coupon) {
  $selected = (in_array($coupon->id, $filterCoupons)) ? ' selected' : '';
?>
...
?>
                    <th><a href="?sortby=field_values->{{ $field['id'] }}&order={{ $reverse_order }}&{{ http_build_query($qsFilterCoupons) }}" class="link text-white">{{ $field['placeholder'] }}</a></th>
<?php } ?>
                    <th class="d-none d-lg-table-cell"><a href="?sortby=created_at&order={{ $reverse_order }}&{{ http_build_query($qsFilterCoupons) }}" class="link text-white">{{ trans('nearby-platform.created') }}</a></th>
...
foreach ($leads as $lead) {
  $sl = \App\Http\Controllers\Core\Secure::array2string(['coupon_id' => $lead->id]);
?>
                  <tr>
                    ...
                    <td class="align-middle text-truncate max-width-200" title="{{ $lead->coupon->title }}">{{ $lead->coupon->title }}</td>
<?php
foreach ($fields as $field) {
    ...
    $('.btn-delete-lead').on('click', function() {
      ...
      swal({
        ...
        if (result.value) {
          ...
          var jqxhr = $.ajax({
            url: "{{ url('dashboard/coupons/leads/delete') }}",
    ...
    $('.btn-delete-selected-leads').on('click', function() {
      ...
      swal({
        ...
        if (result.value) {
          ...
          var jqxhr = $.ajax({
            url: "{{ url('dashboard/coupons/leads/delete/selected') }}",
